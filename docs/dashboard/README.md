...
:::tip Note
Imunify360 DoS protection and Enhanced DoS Protection features are automatically disabled if CSF is active - a warning is shown in Imunify360 UI in that case
:::
...
Enhanced DoS Protection applies the `default_limit` to all ports unless a custom value is defined in `port_limits`.
...
:::tip Note
**JetBackup**: Imunify360 does not use JetBackup as a backup provider. JetBackup is not available in the backup provider list on this page, and Imunify360 does not restore files from JetBackup backups.

JetBackup 5 provides its own integration with Imunify360, which is configured and supported on the JetBackup side. When Imunify360 detects malware, JetBackup can restore the infected file from the most recent backup and/or lock the last backup of the affected account. For details, see the [JetBackup documentation](https://docs.jetbackup.com/v5.4/adminpanel/security.html#imunify360).
:::
...
* <span class="notranslate">**Panel notifications**</span> — the security digests that the Imunify cloud generates and the hosting panel delivers. The <span class="notranslate">_Manage panel notifications_</span> button opens a dialog with the master switch and a switch per message type the product can receive; on Imunify360 that is a single type, <span class="notranslate">_Vulnerable scanner version_</span>, because the other six panel messages are addressed to ImunifyAV/AV+ servers, and everything else on this channel follows the master switch. All switches are enabled by default. See [Panel notifications (iContact)](/features/panel_notifications/). The row is shown only on cPanel and Plesk.
