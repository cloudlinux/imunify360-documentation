...
* **Event notifications** — script hooks that the server runs when a scan starts, finishes or finds malware. Configured on the <span class="notranslate">_Settings → Notifications_</span> tab or via the [CLI](/imunifyav/cli/#notifications-config). **Off by default**, except the malware hook Imunify enables on Plesk.
