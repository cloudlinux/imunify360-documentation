...
<tr><td>Default state</td><td>everything off</td><td>everything on</td></tr>
<tr><td>Timing</td><td>within seconds of the event</td><td>up to a day after the event</td></tr>
...
3. The agent asks the cloud for pending notifications **no more than once every 24 hours**. The check is rate-limited by the modification time of the flag file <span class="notranslate">_/var/imunify360/tmp/icontact_generic_notifications_</span>.
...
* **A configuration change is not reflected immediately.** The cloud reads telemetry that can be up to two days old, and the server asks for messages once a day, so allow up to about 48 hours before concluding that a change had no effect.

## Message types

Seven message types can be sent. Each one is generated independently, has its own cooldown, and can
...
<td><span class="notranslate"><code>MALWARE_SCAN_SCHEDULE.interval</code></span> is set to <span class="notranslate"><code>none</code></span></td>
...
::::tip Note
The <span class="notranslate">_Panel notifications_</span> row appears only when both conditions
hold: the control panel is cPanel/WHM or Plesk, **and** the agent reports a
<span class="notranslate">`DASHBOARD.notifications`</span> section in its configuration. If the row
is missing on a cPanel or Plesk server, check that the agent is up to date:
...
addressed to them, and nothing is reported to the administrator — the hook logs
<span class="notranslate">`Email address '' is invalid`</span> to
<span class="notranslate">_/var/log/imunify360/console.log_</span> and exits successfully. If
...
**See what has been delivered.** Every delivery is logged to
<span class="notranslate">_/var/log/imunify360/console.log_</span> with the full subject and body:
...
**See what the cloud currently has queued for this server.** This is the same endpoint the agent
polls:

<div class="notranslate">

```
curl -s -H "X-Auth: $(cat /var/imunify360/iaid-token)" \
  "https://api.imunify360.com/api/dashboard/v2/events?notification=1&enduser=true" | jq .
```
</div>

Each entry contains the <span class="notranslate">`notification_subject`</span> and
<span class="notranslate">`notification_body_html`</span> that would be delivered.
<span class="notranslate">`{"result": []}`</span> means nothing is queued for this server.

**Ask the cloud again without waiting a day.** Removing the flag file makes the agent poll on its
next cycle:

<div class="notranslate">

```
rm -f /var/imunify360/tmp/icontact_generic_notifications
systemctl restart imunify360        # imunify-antivirus on ImunifyAV/AV+
```
</div>

This re-runs the request, but it does not make the cloud generate a new message: if the type is on
cooldown, or the condition behind it is no longer present in the uploaded telemetry, the response
will still be empty.
